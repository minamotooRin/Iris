    ...
    for country in countries:
        ...
        def process_in_batches(files, batch_size):
            for i in range(0, len(files), batch_size):
                ...
                # captions_batch = captions[i:end_idx]
                # llm_edits_batch = llm_edits[i:end_idx]
                # retrieved_captions_batch = retrieved_captions[i:end_idx]
                # uploaded_files = [upload_to_gemini(path, mime_type="image/jpeg") for path in batch]
                ...
                for src_uploaded_file, tgt_uploaded_file, src_path, tgt_path, file in zip(src_uploaded_files, tgt_uploaded_files, src_paths_batch, tgt_paths_batch, batch):
                    try:
                        ...
                        prompt = [
                            f"You will be given two images. You have to assess how culturally relevant both images are with respect to the culture of {country}.",
                            f"Please explain your reasoning step by step for both images, specifically considering cultural symbols, styles, traditions, or any features that align with the culture of the speaking population of {country}.\n",
                        ...
                        messages = [
                            {"role": "model", "parts": "You are an expert in evaluating the cultural relevance of images."},
