...
if __name__ == "__main__":
    ...
    parser.add_argument("--output_dir", type=str, default='./vlm-based/outputs/gemini', help="Path to the output directory of the target model")
                ...
                for i in range(0, len(files), batch_size):
                    ...
                    for src_uploaded_file, tgt_uploaded_file, src_path, tgt_path, file in zip(src_uploaded_files, tgt_uploaded_files, src_paths_batch, tgt_paths_batch, batch):
                        try:
                            ...
                            prompt = [
                                f"Given the input image and its category '{category}', determine if the comparison image belongs to the same category as the input.\n",
                                ...
                                "{\"reasoning\": ..., \"score\": number}\n",

                            ]

                            messages = [
                                {"role": "model", "parts": "You are an expert in categorizing images based on their content."},
